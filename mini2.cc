...
void addUE(Ptr<LteHelper> lteHelper, NetDeviceContainer ueLteDevs, NetDeviceContainer enbLteDevs, uint16_t i, uint16_t j)
  {
...
main (int argc, char *argv[])
{
  ...
  // change some default attrib utes so that they are reasonable for
  ...
  //                                "MinZ", DoubleValue(5000.0),
  //                                "DeltaX", DoubleValue(100.0),
  //                                "DeltaY", DoubleValue(100.0),
  //                                "DeltaZ", DoubleValue(100.0),
  ...
  ueNodes.Get (3)->GetObject<MobilityModel> ()->SetPosition (Vector (0, yForUe, 0));
  ueNodes.Get (3)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (speed, 0, 0));
  ueNodes.Get (4)->GetObject<MobilityModel> ()->SetPosition (Vector (0, yForUe, 0));
  ueNodes.Get (4)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (speed, 0, 0));*/
  Ptr<ListPositionAllocator> uePositionAlloc = CreateObject<ListPositionAllocator> ();
  for (uint16_t i = 0; i < numberOfUes; i++)
    {
      ueNodes.Get (i)->GetObject<MobilityModel> ()->SetPosition (Vector (0-(i*100), yForUe, 0));
      ueNodes.Get (i)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (speed-(i*2), 0, 0));
    }
...
Simulator::Schedule(Seconds(1), &addUE, lteHelper,ueLteDevs, enbLteDevs,0,0);
Simulator::Schedule(Seconds(2), &addUE, lteHelper,ueLteDevs, enbLteDevs,1,0);
Simulator::Schedule(Seconds(3), &addUE, lteHelper,ueLteDevs, enbLteDevs,2,1);
Simulator::Schedule(Seconds(4), &addUE, lteHelper,ueLteDevs, enbLteDevs,3,1);
Simulator::Schedule(Seconds(5), &addUE, lteHelper,ueLteDevs, enbLteDevs,4,1);
  ...
  // X2-based Handover
  // lteHelper->HandoverRequest (Seconds (6), ueLteDevs.Get (0), enbLteDevs.Get (0), enbLteDevs.Get (1));
  ...
  //Show on NetAnim
  AnimationInterface anim("mini2.xml");
  anim.SetConstantPosition(enbNodes.Get(0), 100.0, 500.0, 0.0);
  anim.SetConstantPosition(enbNodes.Get(1), 600.0, 500.0, 0.0);
  anim.SetConstantPosition(enbNodes.Get(2), 1100.0, 500.0, 0.0);
  Simulator::Run ();
